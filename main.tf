...
resource "intersight_bios_policy" "bios" {
  ...
  intel_vt_for_directed_io = length(
    regexall("(analytical|Data|DSS|HPC|Java)", var.bios_template)
  ) > 0 ? "disabled" : var.intel_vt_for_directed_io                           # Intel VT for Directed IO
  ...
  memory_refresh_rate = length(
    regexall("(DB|M6_HPC)(_tpm)?$", var.bios_template)
  ) > 0 ? "1x" : var.memory_refresh_rate                                # Memory Refresh Rate
  ...
  #+++++++++++++++++++++++++++++++
  c1auto_demotion                    = var.c1auto_demotion                    # C1 Auto Demotion
  c1auto_un_demotion                 = var.c1auto_un_demotion                 # C1 Auto UnDemotion
  ...
  cpu_perf_enhancement = length(                                              # Enhanced CPU Performance
    regexall("M6_[a-zA-Z\\_]+(_tpm)?", var.bios_template)
  ) > 0 ? "Auto" : var.cpu_perf_enhancement
  llc_alloc = length(
    regexall("M6_(HPC|relational_DB)(_tpm)?", var.bios_template)
  ) > 0 ? "enabled" : var.llc_alloc             # LLC Dead Line
  upi_link_enablement = var.upi_link_enablement # UPI Link Enablement
  upi_power_management = length(
    regexall("M6_(HPC|relational_DB)(_tpm)?", var.bios_template)
  ) > 0 ? "enabled" : var.upi_power_management  # UPI Power Management
  virtual_numa        = var.virtual_numa        # Virtual Numa
  xpt_remote_prefetch = var.xpt_remote_prefetch # XPT Remote Prefetch
  ...
  cpu_power_management = length(
    regexall("^(DSS|Java|OLTP|Virtualization)(_tpm)?$", var.bios_template)
  ) > 0 ? "custom" : var.cpu_power_management       # Power Technology
  ...
  energy_efficient_turbo = length(
    regexall("^M6_(relational_DB|HPC)(_tpm)?$", var.bios_template)
  ) > 0 ? "disabled" : var.energy_efficient_turbo                     # Energy Efficient Turbo
  ...
  intel_hyper_threading_tech = length(
    regexall("^((M6_)?(analytical_DB|HPC))(_tpm)?$", var.bios_template)
  ) > 0 ? "disabled" : var.intel_hyper_threading_tech # Intel HyperThreading Tech
  ...
  intel_virtualization_technology = length(           # Intel(R) VT
    regexall("^(HPC|Java)(_tpm)?$", var.bios_template)
  ) > 0 ? "disabled" : var.intel_virtualization_technology
  ...
  memory_inter_leave = length(
    regexall("(relational_DB|M6_HPC)?$", var.bios_template)
  ) > 0 ? "1 Way Node Interleave" : var.memory_inter_leave # Intel Memory Interleaving
  package_cstate_limit = var.package_cstate_limit          # Package C State Limit
  patrol_scrub = length(
    regexall("(DB|M6_HPC)(_tpm)?$", var.bios_template)
  ) > 0 ? "custom" : var.patrol_scrub               # Patrol Scrub
  patrol_scrub_duration = var.patrol_scrub_duration # Patrol Scrub Interval
  processor_c1e = length(                           # Processor C1E
    regexall("(DSS|M6_HPC|Java|OLTP|DB|Virtualization)(_tpm)?", var.bios_template)
  ) > 0 ? "disabled" : var.processor_c1e
  processor_c3report = length(
    regexall("^(DSS|Java|OLTP|Virtualization)(_tpm)?$", var.bios_template)
  ) > 0 ? "disabled" : var.processor_c3report # Processor C3 Report
  processor_c6report = length(
    regexall("^(DSS|M6_HPC|Java|OLTP|Virtualization)(_tpm)?$", var.bios_template)
  ) > 0 ? "disabled" : var.processor_c6report # Processor C6 Report
  processor_cstate = length(
    regexall("^(DSS|Java|OLTP|Virtualization)(_tpm)?$", var.bios_template)
  ) > 0 ? "disabled" : var.processor_cstate   # CPU C State
  ...
  work_load_config = length(                # Workload Configuration
    regexall("(M6_)?(analytical_DB|Data|HPC)(_tpm)?", var.bios_template)
  ) > 0 ? "Balanced" : var.work_load_config
  xpt_prefetch = length(
    regexall("M6_(HPC|relational_DB)(_tpm)?", var.bios_template)
  ) > 0 ? "enabled" : var.xpt_prefetch # XPT Prefetch
  ...
  #+++++++++++++++++++++++++++++++
  cpu_pa_limit                    = var.cpu_pa_limit                    # CPU PA to 46 Bits
